<template>
	<view class="swiper-list-card swiper-list">
		<view class="swiper-main-card swiper-main">
		</view>
	</view>
...
<script>
	...
	const Swiper = function(obj) {
		...
		this.intervalTime = this.aniTime + obj.intervalTime || 99999;
		this.nowIndex = 2;
	...
	Swiper.prototype = {
		init: function() {
			...
			if (this.imgArr.length > 2) {
				resImgArr = [this.imgArr[this.imgArr.length - 2], this.imgArr[this.imgArr.length - 1], ...this
					.imgArr, this.imgArr[0], this.imgArr[1]
				];
				...
				if (this.nowIndex === 1) {
					...
					setTimeout(function() {
						...
						this.mainDom.style.left =
							`${-(parseInt(this.imgDoms[this.nowIndex].style.left) - this.diffLen - this.gap)}px`;
					}.bind(this), aniTime)
				} else {
					this.setScale()
				}
	...
	export default {
		...
		data() {
			return {
				imgArr: [{
						url: '#',
						imgPath: '/static/images/基础.png'
					},
					{
						url: '#',
						imgPath: '/static/images/初级.png'
					},
					{
						url: '#',
						imgPath: '/static/images/中级.png'
					},
					{
						url: '#',
						imgPath: '/static/images/高级.png'
					},
				]
		...
		mounted() {
			new Swiper({
				imgArr: this.imgArr, // 图片数组
				imgWidth: 120, // 图片宽度
				aniTime: 1000, // 动画切换时间
				intervalTime: 99999, // 停留的时间
				scale: 0.8, // 图片缩放
