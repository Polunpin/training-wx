<template>
  <div class="index-container" v-if="!pageLoading">
    ...
    <div class="module">
      <!-- 当前进度 -->
      <div class="module-current" v-if="Object.keys(dataInfo).length && dataInfo.level4Dir">
        <div class="current-info">
          <span class="info-title">当前进度：</span>
          {{ dataInfo.level4Dir }}
        </div>
        <div class="current-btn"
             @click="jumpPage(`/pages/studyView/studyInfo?level2Dir=${dataInfo.level2Dir}&level3Dir=${dataInfo.level3Dir}&level4Dir=${dataInfo.level4Dir}`)">
          继续学习
        </div>
      </div>
      <!-- 模块列表 -->
